...
function PlannerCalendar({
  ...
  if (!groups.length) {
    // Four distinct empty cases:
    // - Plan has saved items, details still loading → temporary spinner copy.
    // - Plan has saved items, details loaded, every saved id is orphaned in
    //   the live program → offer a cleanup CTA (this is the common case
    //   when an organizer edits/recreates schedule items and the original
    //   ids vanish, leaving the user's `Точки: N` stuck without a render).
    // - Plan has saved items, details still loading but none matched yet →
    //   generic refresh hint.
        ...
        <View style={styles.emptyCalendar}>
          ...
          <Pressable
            ...
            {cleaningUp ? (
              ...
              <Text style={styles.emptyCleanupBtnText}>Изчисти {orphanedCount} {orphanedCount === 1 ? 'точка' : 'точки'}</Text>
            )}
          </Pressable>
        </View>
      );
...
export default function PlanScreen() {
   ...
   * trust this list after every detail query has settled, otherwise an in-flight
   * fetch would look like a missing item and we'd offer to wipe legitimate
   * plan rows.
   */
  ...
  const handleCleanupOrphans = async () => {
    ...
    try {
      // POST /api/plan/items toggles — calling it with an existing id deletes
      // the row, so we issue one call per orphan id. They are usually a
      // handful at most; sequential is fine and easier to recover from on
      // partial failures.
